...
import com.smc.psitem.service.PsItemFormValidator;
import com.smc.psitem.service.PsItemServiceImpl;
...
@Controller
@Slf4j
@RequestMapping("/login/admin/psitem")
@SessionAttributes(value= {"SessionScreenState", "SessionUser"})
public class PsItemController extends BaseController {


	@Autowired
	PsItemServiceImpl service;

	@Autowired
	PsItemFormValidator validator;
	...
	private final String DIR="/tmp/smcps_data";
	...
	public ModelAndView list(
			ModelAndView mav,
			@ModelAttribute("psItemForm") PsItemForm myform,
		...
		mav.setViewName("/login/admin/psitem/index");
	...
	public ModelAndView post(
			...
			@Validated @ModelAttribute("psItemForm") PsItemForm form,
			BindingResult result) {
		// XXX BindingResultは@Validatedの直後の引数にする

		// Set view
		mav.setViewName("/login/admin/psitem/result");
		...
		if (!result.hasErrors()) {
            ...
            String fullpath = dir+"/"+"psitem_"+form.getLang()+".csv";
